...
def parse_cachevault_or_bbu(cv_text: str, bbu_text: str) -> dict:
    ...
    def _extract_absent_phrase(text: str, default_phrase: str) -> str:
        ...
        if m:
            # keep exact phrase (as requested earlier in v3 behavior)
            return m.group(1)
    ...
    # BBU
    if bbu_text:
        if re.search(r"(battery|bbu)\s+(is\s+absent!|not\s+present|absent)", bbu_text, re.IGNORECASE) or re.search(
            r"no\s+battery\s+present", bbu_text, re.IGNORECASE
        ):
...
def parse_vds_from_topology_fallback(dall_text: str) -> list:
    ...
    for line in topo.splitlines():
        ...
        # RAID rows usually have EID as '-' (not a drive row)
        if m.group("eid") != "-":
            continue
    ...
    # Deduplicate within topology fallback
    uniq, seen = [], set()
    for vd in vds:
...
# ------------------------------------------------------------
st.set_page_config(page_title="StorCLI Log Dashboard (v4)", layout="wide")
st.title("📊 StorCLI Log Dashboard (v4)")
st.markdown("Upload StorCLI `.txt` files or a `.zip` archive. Search logs and view a clean Summary at the end.")
...
# ------------------------------------------------------------
# Summary Panel (END) — CLEAN TEXT (no quotes, no extra spacing)
# ------------------------------------------------------------
st.markdown("---")
st.subheader("🧾 Summary")

def render_compact_section(header: str, lines: list[str]):
    """Compact section: heading + st.text (no extra blank spacing, no JSON quotes)."""
...
if not all_files:
    ...
    render_compact_section(
        ...
        [
            ...
            f"PDs: Total={pd_summary['total_pds']} | Failed/Offline={pd_summary['failed_pds']} | MediaErr>0={pd_summary['media_error_pds']} | OtherErr>0={pd_summary['other_error_pds']} | PredFail>0={pd_summary['predictive_failure_pds']}",
        ],
    )
    ...
    if not abnormal:
        st.text(header_line + "\n" + "No abnormal PD counters/states detected (all counters are 0 and states are Online/Optimal where available).")
    else:
        lines = [header_line]
        for pd in abnormal:
            media = pd.get("media_error_count") or 0
